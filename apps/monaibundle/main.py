...
class SegmentationApp(MONAILabelApp):
    def __init__(self, app_dir, studies, conf):
        ...
        #     get_bundle_models(app_dir, conf, conf_key="bundles") if conf.get("bundles") else None
        # )
...
def main():
    ...
    logging.basicConfig(
        level=logging.INFO,
        format="[%(asctime)s] conf[%(process)s] [%(threadName)s] [%(levelname)s] (%(name)s:%(lineno)d) - %(message)s",
        datefmt="%Y-%m-%d %H:%M:%S",
        force=True,
    )
    ...
    app = SegmentationApp(app_dir, studies, {"preload": "true", "models": "spleen_ct_segmentation"})
    train(app)
